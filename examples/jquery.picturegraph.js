(function() {
  $.fn.extend({
    pictureGraph: function(cols, rows, color, array) {
      var body_segment, col, height, row, span_height, span_width, width, _ref, _ref2;
      width = this.width();
      height = this.height();
      this.append($('<div>').addClass('pictureGraph_container').css('width', width + 3).css('height', height).css('margin-left', 1).css('margin-top', 3));
      ...
      return $('.pictureGraph_segment').live('click', function() {
        return alert($(this).data('grid_point'));
      });
    }
  });
}).call(this);
